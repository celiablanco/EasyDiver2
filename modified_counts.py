    ...
    if not any(fnmatch.fnmatch(file, in_format) for file in os.listdir(counts_dir)):
        ...
        for i in range(1, max_round):
            ...
            if not neg_files_exist:
                run_enrichment_analysis(out_file=glob.glob(os.path.join(counts_dir, str(i) + "-out*" + "_" + counts_type + ".txt"))[0], res_file=f"{outdir}/modified_counts/{i}-res.csv")
            else:
                run_enrichment_analysis(out_file=glob.glob(os.path.join(counts_dir, str(i) + "-out*" + "_" + counts_type + ".txt"))[0], res_file=f"{outdir}/modified_counts/{i}-res.csv", neg_file=glob.glob(os.path.join(counts_dir, str(i + 1) + "-neg*" + "_" + counts_type + ".txt")))
        ...
        for i in range(1, max_round + 1):
            ...
            neg_files_exist = any(fnmatch.fnmatch(file, neg_format) for file in os.listdir(counts_dir))
            if not neg_files_exist:
                run_enrichment_analysis(out_file=glob.glob(os.path.join(counts_dir, str(i) + "-out*" + "_" + counts_type + ".txt"))[0], in_file=glob.glob(os.path.join(counts_dir, str(i) + "-in*" + "_" + counts_type + ".txt"))[0], res_file=f"{outdir}/modified_counts/{i}-res.csv")
            else:
                run_enrichment_analysis(out_file=glob.glob(os.path.join(counts_dir, str(i) + "-out*" + "_" + counts_type + ".txt"))[0], in_file=glob.glob(os.path.join(counts_dir, str(i) + "-in*" + "_" + counts_type + ".txt"))[0], res_file=f"{outdir}/modified_counts/{i}-res.csv", neg_file=glob.glob(os.path.join(counts_dir, str(i) + "-neg*" + "_" + counts_type + ".txt"))[0])
