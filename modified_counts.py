...
def next_round_file(input_str):
    ...
    if match:
        ...
        incremented_number = str(
            number + 1
        )  # Increment the integer and convert it back to string
    ...
    for file in os.listdir(directory):
        if file.startswith(file_pre) and file.endswith(".txt"):
            return os.path.join(
                directory, file
            )  # directory + input_str[last_slash_index + 1:].replace(match.group(0), file)
    return None
...
def run_enrichment_analysis(out_file, in_file=None, res_file=None, neg_file=None):
    print(out_file, in_file, res_file, neg_file)
    start = time()
    ...
    if in_file is None:
        ...
        in_file = (
            out_file  # Treat current out file as the input file for the next round
        )
        out_file = next_round_file(
            in_file
        )  # ex: If the file is named 9-out.txt, the next_round_file is named 10-out.txt
        if (
            not os.path.exists(out_file)
            or neg_file is not None
            and not os.path.exists(neg_file)
        ):
            print(
                "Hmm... it seems as if there is no out file after this one. Would you like to specify an in file?"
            )
    ...
    with open(res_file, "w", newline="") as csv_file:
        ...
        for seq in all_dict[
            -1
        ]:  # Originally 2. Calculate each sequence's a_in, f_in, a_out, etc. stats
            ...
            c_in_range = (
                [0, 0]
                if c_in == 0
                else [max(c_in - c_in_boot[1], 1), c_in + c_in_boot[1]]
            )
            ...
            if seq in unique_sequences.keys():
                print(
                    ...
                    + seq
                    + '" '
                    + format_bootstrap(c_post_range, "a")
                    + " times with "
                    + format_bootstrap(f_post_range, "f")
                    + " frequency."
                )
            else:
...
def find_enrichments():
    ...
    if not any(fnmatch.fnmatch(file, in_format) for file in os.listdir(counts_dir)):
        ...
        for i in range(1, max_round):
            ...
            neg_files_exist = any(
                fnmatch.fnmatch(file, neg_format) for file in os.listdir(counts_dir)
            )
            if not neg_files_exist:
                run_enrichment_analysis(
                    out_file=glob.glob(
                        os.path.join(
                            counts_dir, str(i) + "-out*" + "_" + counts_type + ".txt"
                        )
                    )[0],
                    res_file=f"{outdir}/modified_counts/{i}-res.csv",
                )
            else:
                run_enrichment_analysis(
                    out_file=glob.glob(
        ...
        for i in range(1, max_round + 1):
            ...
            if not neg_files_exist:
                run_enrichment_analysis(
                    ...
                    in_file=glob.glob(
                        os.path.join(
                            counts_dir, str(i) + "-in*" + "_" + counts_type + ".txt"
                        )
                    )[0],
                    res_file=f"{outdir}/modified_counts/{i}-res.csv",
                )
                ...
                run_enrichment_analysis(
                    ...
                    neg_file=glob.glob(
                        os.path.join(
                            counts_dir, str(i) + "-neg*" + "_" + counts_type + ".txt"
                        )
                    )[0],
                )
