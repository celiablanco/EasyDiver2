...
def bootstrap(count, total, depth=1002, smoothing=1, seed=42):
    ...
    if total != 1:  # Not a decimal
        ...
        bootstraped_counts = np.random.binomial(
            smoothed_total, smoothed_count / smoothed_total, size=depth
        )
    else:
        bootstraped_counts = np.random.binomial(
            total * 10000, count / total, size=depth
        )
    ...
    return [
        count,
        bootstraped_std / 10000 if total == 1 else round(bootstraped_std),
    ]  # List of sampled count and the margin of error up or down
